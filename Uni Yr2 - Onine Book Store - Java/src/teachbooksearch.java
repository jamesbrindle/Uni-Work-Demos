//this servlet seaches for books with details put into a form in 'teachersearch' and diplays the results
...
public class teachbooksearch extends common {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (validated) { //if teacher is logged in...
            ...
            int rowCount = 1;
            //get details entered into a form in 'teachersearch
            ...
            try { //get book details from database
                ...
                while (rs1.next()) { //show results...
                    ...
                    for (int i = 1; i < columnCount + 1; i++) {
                        ...
                        if (i == 1) {
                            out.println(
                                    "<td rowspan='5' valign='middle'><form action=\"editbook\" method=\"get\">"
                                            + "<input type=\"hidden\" name=\"bookid\" value=\""
                                            + rs1.getString("bookid") + "\">"
                                            + "<input type=\"submit\" value=\"edit book\" &rarr;\">"
                                            + "</form>" + "<br>"
                                            + "<form action=\"addreadinglist\" method=\"get\">"
                                            + "<input type=\"hidden\" name=\"bookid\" value=\""
                                            + rs1.getString("bookid") + "\">"
                                            + "<input type=\"submit\" value=\"add to reading list\" &rarr;\">"
                                            + "</form>" + "<br>"
                                            + "<form action=\"removebook\" method=\"get\">"
                                            + "<input type=\"hidden\" name=\"bookid\" value=\""
                                            + rs1.getString("bookid") + "\">"
                                            + "<input type=\"submit\" value=\"remove book\" &rarr;\">"
                                            + "</form>" + "<br>" + "</td>");
                            out.println("</tr>");
                        } else {
                            out.println("</tr>");
                        }
                        rowCount++; //increment rowcount if rows are returned (if search found books)
                ...
                if (rowCount == 1) { //if rows returned (books found) ...
                    out.println("<font face='Arial, Helvetica, sans-serif'>");
                    out.println(
                            "<br>your request had no matches, try using fewer conditions");
                } else { //if no books found...
                    out.println(
                            "<br><font face='Arial, Helvetica, sans-serif'>please refine your search if your requested book does not appear");
                }
                out.println("<br><a href='teachsearch'>back to search</a>");
                out.println("</font>");
